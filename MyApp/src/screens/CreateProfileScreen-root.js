...
const CreateProfileScreen = ({navigation}) => {
  ...
  //   // Kiểm tra thông tin
  //   if (fullName === '' || date === '' || selectedGender === '') {
  //     Alert.alert('Thông báo', 'Vui lòng nhập đầy đủ thông tin');
  //     return;
  //   }

  //   // Lưu thông tin vào bộ nhớ cục bộ
  //   console.log('continue: fullName:', fullName);
  //   try {
  //     await AsyncStorage.setItem('fullName', fullName);

  //     // di chuyển đến màn hình Main
  ...
  const handleContinue = async () => {
    // Code lưu thông tin vào bộ nhớ cục bộ
    ...
    if (avatarUri) {
      ...
      try {
        const response = await axios.post('http://192.168.2.41:3000/upload', formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
            // Bạn cần thêm các header khác tùy thuộc vào yêu cầu của backend
          },
        });
